...
def preprocess_data(df):
    ...
    if not cluster_features:
        # CRITICAL ERROR: We have no features to cluster on!
        st.error("Error: The uploaded file does not contain any of the required columns (like 'income', 'year_birth', 'total_spent', etc.) to perform an analysis.")
...
def find_clusters(df_scaled, optimal_k=4):
    ...
    # We will hardcode K=4 for this app's demo
...
def profile_clusters(df_clean, cluster_labels, cluster_features):
    ...
    # Base profile on the features used for clustering
    profile_features = cluster_features.copy()
    
    # Add other interesting features if they exist
...
def train_model(df_clean, cluster_features):
    ...
    # Start with the features used for clustering
    features = cluster_features.copy()
    
    # Add other predictive features if they exist
    ...
    return rf_model, accuracy, report, features # Return the features we actually used
...
if uploaded_file:
    ...
    if st.sidebar.button("Run Analysis", type="primary"):
        with st.spinner("Processing your data... This may take a moment."):
            ...
            st.session_state.cluster_features = cluster_features # Save this
            ...
            st.session_state.model_features = model_features # Save the features the model was trained on
...
if st.session_state.pipeline_complete:
    # --- ADD THIS DICTIONARY ---
    # Look at the "Segment Profiles" table in your app to decide on the names.
    # These are just examples!
    CLUSTER_NAMES = {
        0: "VIP (High Income, High Spend)",
        1: "Bargain Hunter (Low Spend, High Deals)",
        2: "Average Customer (Medium)",
        3: "At-Risk (Low Recency/Spend)"
    }
    # --- END OF ADDITION ---
    ...
    * **Clustering was based on these features:** `{(', '.join(st.session_state.cluster_features))}`
    ...
    # Check if the model was trained on the "standard" set of features
    # If not, we can't show the form
    ...
    # Check if all expected features are in the model's features
    # This allows the model to have EXTRA features, but it must have at least these
    if expected_feature_set.issubset(model_feature_set):
        with st.form("prediction_form"):
            ...
            if submitted:
                # Calculate total_spent based on form inputs
                # This assumes 'mnt...' columns beyond wine and meat are 0 for this prediction
                total_spent = mnt_wines + mnt_meat # Simple, but works for the demo
                
                # Gather all data in the correct order
                input_data_dict = {
                    'age': age, 'income': income, 'total_spent': total_spent, 
                    'family_size': family_size, 'years_customer': years_customer,
                    'mntwines': mnt_wines, 'mntmeatproducts': mnt_meat,
                    'numdealspurchases': num_deals, 'numwebpurchases': num_web,
                    'numcatalogpurchases': num_catalog, 'numstorepurchases': num_store
                }
                
                # Filter down to only the features the model was trained on
                input_data = [input_data_dict[feat] for feat in st.session_state.model_features]
                ...
                # Get the predicted cluster number
                cluster_num = prediction[0]

                # Get the personality name from our dictionary
